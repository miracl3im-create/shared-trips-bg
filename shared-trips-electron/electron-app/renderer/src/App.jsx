...
import { Car, Users, MessageCircle, ArrowLeftRight, MapPin, Calendar, Clock, User, PlusCircle } from "lucide-react";
...
const Pill = ({ icon: Icon, children }) => (
  <span className="inline-flex items-center gap-1 px-2 py-1 rounded-full bg-white/70 shadow border text-xs">
...
function FitBounds({ fromCity, toCity }) {
  ...
  useEffect(() => {
    if (fromCity && toCity) {
      map.fitBounds([[fromCity.lat, fromCity.lng], [toCity.lat, toCity.lng]], { padding: [60, 60] });
    }
  }, [fromCity, toCity, map]);
  return null;
}
...
export default function App() {
  ...
  const filteredTrips = useMemo(() =>
    trips.filter(t =>
      (!fromName || t.from === fromName) &&
      (!toName   || t.to   === toName) &&
      (dateFilter === "all" || t.date === dateFilter)
    ), [trips, fromName, toName, dateFilter]
  );
  const datesForFilter = useMemo(() =>
    ["all", ...Array.from(new Set(trips.map(t => t.date))).sort()], [trips]
  );
  ...
  useEffect(() => {
    ...
    s.emit('chat:join', selectedTrip.id);
    const handler = (m)=>setChat(prev=>[...prev,m]);
  ...
  useEffect(() => {
    if (activeTripId) {
      fetch(`${API_BASE}/trips/${activeTripId}/chat`).then(r=>r.json()).then(setChat).catch(()=>setChat([]));
    }
  }, [activeTripId]);
  ...
  async function createTrip() {
    ...
    alert('Курсът е създаден');
    setNewDate(''); setNewTime(''); setSeats(3);
    const params = new URLSearchParams();
    if (fromName) params.set("from", fromName);
    if (toName)   params.set("to", toName);
    if (dateFilter !== "all") params.set("date", dateFilter);
    const data = await fetch(`${API_BASE}/trips?${params}`).then(r=>r.json());
    setTrips(data);
  }

  async function requestToJoin(tripId){
    const r = await fetch(`${API_BASE}/trips/${tripId}/request`, {
  ...
  async function sendChat(){
    if (!selectedTrip || !chatInput.trim()) return;
    await fetch(`${API_BASE}/trips/${selectedTrip.id}/chat`, {
      method:'POST', headers:{'Content-Type':'application/json'},
      body: JSON.stringify({ text: chatInput.trim(), userId: user.id, userName: user.name })
    });
    setChatInput('');
  }
  ...
  return (
    <div className="w-full h-screen flex bg-gradient-to-b from-sky-50 to-slate-50 text-slate-800">
      {/* Sidebar */}
      <div className="w-[400px] border-r bg-white/70 backdrop-blur p-4 flex flex-col gap-3 overflow-y-auto">
        <div className="flex items-center gap-2">
          <div className="p-2 rounded-2xl bg-sky-100"><Car/></div>
          <div>
            <h1 className="text-xl font-semibold">Споделени пътувания</h1>
            <p className="text-xs text-slate-500">Локално приложение</p>
          </div>
        </div>

        <div className="rounded-2xl border bg-white p-3">
          <div className="grid gap-2">
            <input className="border rounded-xl px-3 py-2" placeholder="Твоето име (за чат)"
        ...
        <div className="grid grid-cols-5 gap-2 items-end">
          <div className="col-span-2">
            ...
            <select className="w-full mt-1 p-2 border rounded-xl" value={fromName} onChange={e=>setFromName(e.target.value)}>
              {cities.map(c=><option key={c.name}>{c.name}</option>)}
            </select>
          </div>
          <div className="flex items-center justify-center pb-2">
            <button onClick={()=>{ const a=fromName; setFromName(toName); setToName(a); }} className="p-2 border rounded-xl hover:bg-slate-50">
              <ArrowLeftRight/>
            </button>
          </div>
          <div className="col-span-2">
            <label className="text-xs">До</label>
            <select className="w-full mt-1 p-2 border rounded-xl" value={toName} onChange={e=>setToName(e.target.value)}>
              {cities.map(c=><option key={c.name}>{c.name}</option>)}
            </select>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-2">
          <div>
            ...
            <select className="w-full mt-1 p-2 border rounded-xl" value={dateFilter} onChange={e=>setDateFilter(e.target.value)}>
              {datesForFilter.map(d=><option key={d} value={d}>{d==='all'?'Всяка дата':d}</option>)}
            </select>
          </div>
        </div>
      ...
      <div className="flex-1 grid grid-rows-[1fr, minmax(220px, 40%)]">
        <div className="relative">
          <MapContainer center={[42.6977, 23.3219]} zoom={7} className="h-full w-full" scrollWheelZoom>
            ...
            {cities.map(c => (
              <CircleMarker key={c.name} center={[c.lat, c.lng]} radius={6} pathOptions={{ color: c.name===fromName?'#0ea5e9': c.name===toName?'#22c55e':'#334155', weight: 2 }}>
                <Popup>
                  <div className="text-sm font-medium flex items-center gap-2"><MapPin size={16} /> {c.name}</div>
                </Popup>
              </CircleMarker>
            ))}
            {fromCity && toCity && (
              <Polyline positions={[[fromCity.lat, fromCity.lng],[toCity.lat, toCity.lng]]} pathOptions={{ weight: 5, opacity: 0.6 }} />
            )}
        ...
        <div className="border-t bg-white/80 backdrop-blur grid grid-cols-2 gap-0">
              ...
              {filteredTrips.map(t => {
                ...
                return (
                  <motion.div
                    key={t.id}
                    layout
                    onDoubleClick={() => requestToJoin(t.id)}
                    className={`rounded-2xl border p-3 bg-white shadow-sm cursor-pointer transition ${activeTripId===t.id?"ring-2 ring-sky-300":""}`}
          ...
          <div className="border-l p-4 flex flex-col">
            <div className="flex items-center justify-between">
              ...
              {selectedTrip ? (
                <div className="text-xs text-slate-500">{selectedTrip.from} → {selectedTrip.to} · {selectedTrip.date} {selectedTrip.time}</div>
              ) : (
                <div className="text-xs text-slate-500">Избери курс</div>
              )}
            </div>
            ...
            <div className="mt-2 flex-1 overflow-y-auto rounded-xl border bg-white p-3">
              {selectedTrip ? (
                chat.length ? (
                  ...
                  <div className="h-full grid place-items-center text-slate-400 text-sm">Още няма съобщения</div>
                )
              ) : (
            ...
            <div className="mt-2 flex gap-2">
              <input
                ...
                placeholder={selectedTrip? "Напиши съобщение…" : "Избери курс"}
                value={chatInput}
                onChange={e=>setChatInput(e.target.value)}
                onKeyDown={e=>{ if (e.key === 'Enter') sendChat(); }}
                disabled={!selectedTrip}
              />
              <button className="px-4 py-2 rounded-xl border bg-white hover:bg-slate-50" onClick={sendChat} disabled={!selectedTrip}>
                Изпрати
              </button>
            </div>
          </div>
